  ...
  export default {
    ...
    watch: {
      ...
      selectedCategory: function(){
        ...
        console.log('filter location changed')
      },
    },
    methods: {
      checkAllLocation(){
      ...
      fetchTags: async function(){
        ...
        this.categories.map((category) => {
          ...
          const findTag = data.data.tags.find(tag => tag.name == categoriesTitle)
        ...
        this.locations.map((location) => {
          ...
          const findTag = data.data.tags.find(tag => tag.name == locationsTitle)
      ...
      fetchSuggestions: async function() {
        ...
        console.log(optCategories, optLocations)
        console.log(
          "\n search text: " + this.searchText, 
          "\n by category: " + optCategories.join(), 
          "\n by Location: " + optLocations.join());
          ...
          if (this.searchText) {
            ...
            if(data.stories.length >= 1) {
              this.getPost = true;
              console.log("Are there a post?", this.getPost)
            } else {
              this.getPost = false;
              console.log("Are there a post?", this.getPost)
            }
  ...
  <div v-editable="blok" id="projectsFilter" class="container">
    <div class="vue-app-wrapper">
      <div class="filter-wrapper">
        <!-- <div class="row">
          <div>Locations: {{ selectedLocation.map(location => location.value) }}</div>
          <div>Categories: {{ selectedCategory.map(category => category.value) }}</div>
        </div> -->
        <div class="row">
          <div class="col-md-6 col-lg-3"><h1>Projects</h1></div>
          <div class="col-md-6 col-lg-9">
            <div id="filters-wrapper" :class="showFilterOnMobile ? 'active' : ''">
              <a @click="showFilterOnMobile = !showFilterOnMobile" href="#" class="close-icon d-md-none">
                <img src="https://evercam.io/wp-content/themes/evercam/img/filter-close.svg" alt="Close"/>
              </a>  
              <h3 class="d-md-none">Filter by</h3>
              <div class="select-dropdown">
                <button id="locations" type="button" class="" @click="showFilterLocation = !showFilterLocation">Location</button>
                <ul v-if="showFilterLocation" class="categories-list" style="display:block">
                  <li class="categories-list-item">
                    <div class="custom-control custom-checkbox">
                      ...
                      />
                      <label for="checkbox-all-country" class="custom-control-label">ALL</label>
                    </div>
                  </li>
                  <li v-for="(location, key) in locations" :key="location.value" class="categories-list-item">
                    <div class="custom-control custom-checkbox">
                      <input
                        ...
                        class="custom-control-input"
                      />
                      <!-- <label for="checkbox-country-0" class="custom-control-label"></label> -->
                      <label :for="'checkbox-country-' + key" class="custom-control-label">
                        <span class="text">{{ location.value }}</span>
                        <span class="badge badge-secondary float-right">{{ location.total }}</span>
                      </label>
                    </div>
                  </li>
                </ul>
              ...
              <div class="select-dropdown">
                <button id="categories" type="button" class="" @click="showFilterCategory = !showFilterCategory">Categories</button>
                <ul v-if="showFilterCategory" class="categories-list" style="display:block">
                  <li class="categories-list-item">
                    <div class="custom-control custom-checkbox">
                      <input
                        ...
                        class="custom-control-input"
                      />
                      <label for="checkbox-all-category" class="custom-control-label">ALL</label>
                    </div>
                  </li>
                  <li v-for="(category, key) in categories" :key="category.value" class="categories-list-item">
                    <div class="custom-control custom-checkbox">
                      <input
                      ...
                      <label :for="'checkbox-category-'+key" class="custom-control-label">
                        <span class="text">{{ category.value }}</span>
                        <span class="badge badge-secondary float-right">{{ category.total }}</span>
                      </label>
                    </div>
                  </li>
                </ul>
              ...
              <div class="search-box">
                <div class="input-group">
                  <SearchProject :search="fetchSuggestions" v-model="searchText"/>
      ...
      <div class="switches-wrapper">
        <div id="toogle-switch">
          <a href="#" class=""><span><font-awesome-icon :icon="['fas', 'map-marker-alt']" /></span>Map View</a>
          <a href="#" class="active"><span><font-awesome-icon :icon="['fas', 'th-large']" /></span>GridView</a>
          <a @click="showFilterOnMobile = !showFilterOnMobile" href="#" class="d-md-none"><span><font-awesome-icon :icon="['fas', 'filter']"/></span>Filters</a>
        </div>
      </div>
